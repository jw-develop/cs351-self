...
struct cacheresponse getcache(char* filename);
...
void handleconnection(const int clientfd, char* user_agent) {
    ...
    /* the file descriptor for the cachefd */
    int cachefd;
    ...
    int i;
    /* the cached response */
    struct cacheresponse cachedresponse;
    ...
    strcat(filename, uri);
    strcat(filename,".cache");
    for (i = 6; filename[i] != '\0' && i < MAXLINE; i++) {
        if (filename[i] == '/') 
            filename[i] = '_';
    }

    debug("set filname to: %s\n", filename);

    debug("method: %s uri: %s version: %s\n", method, uri, version);

    cachedresponse = getcache(filename);
    if (cachedresponse.length != -1) {
        Rio_writen(clientfd, cachedresponse.response, cachedresponse.length);
        free(cachedresponse.response);
    } else {
        ...
        /* save the response the cache */
        debug("writing the response to the cache\n");

        debug("writing response to cache file: %s \n", filename);
        cachefd = open(filename, O_WRONLY | O_CREAT, 0777);
        debug("created file\n");
        Rio_writen(cachefd, response.headers, strlen(response.headers));
        debug("wrote headers\n");
        Rio_writen(cachefd, response.body, response.bodysize);
        debug("wrote body\n");
        debug("done writing response to cache file: %s \n", filename);
        Close(cachefd);
...
struct cacheresponse getcache(char* filename) {
    /* the file descriptor of the cache file */
    int fd;
    /* the response read from the cache */
    struct cacheresponse cachedresponse;
    cachedresponse.response = malloc(MAX_OBJECT_SIZE);
    fd = open(filename, O_RDONLY, 0777);
    if (fd == -1) {
        cachedresponse.length = -1;
    } else {
        cachedresponse.length = read(fd, cachedresponse.response, MAX_OBJECT_SIZE);
    }
